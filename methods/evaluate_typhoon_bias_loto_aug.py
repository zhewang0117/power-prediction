...
def run_loto(df: pd.DataFrame, capacity: float, candidate_feats: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    events = df.groupby("台风编号")["target_time"].min().sort_values().index.tolist()
    if len(events) < 4:
        raise RuntimeError("Need at least 4 events for LOTO")
    ...
    for test_event in events:
        ...
        p_corr = np.clip(apply_feature_binner(test_df, best_feat, q, gb, lb), 0.0, capacity)

        m_base = metrics(y_true, p_base, capacity)
        m_corr = metrics(y_true, p_corr, capacity)

        event_name = str(test_df["台风中文名称"].dropna().iloc[0]) if "台风中文名称" in test_df.columns and test_df["台风中文名称"].notna().any() else ""
...
def main():
    ...
    p.add_argument("--out-dir", default="loto_results_aug")
    args = p.parse_args()

    base_feats = ["移动速度", "场站距离_km", "风速", "气压", "台风等级"]
    aug_feats = [
        "rmax",
        "b",
        "hollandvg",
    ...
    candidate_feats = base_feats + aug_feats

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    df = build_dataset(args.pred_glob, args.typhoon_path)
    fold_df, summary_df = run_loto(df, args.capacity, candidate_feats)

    fold_path = out_dir / "typhoon_loto_fold_metrics_aug.csv"
    summary_path = out_dir / "typhoon_loto_summary_aug.csv"
    feat_path = out_dir / "selected_feature_counts_aug.csv"

    fold_df.to_csv(fold_path, index=False, encoding="utf-8-sig")
    summary_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    fold_df["selected_feature"].value_counts().rename_axis("feature").reset_index(name="count").to_csv(
        feat_path, index=False, encoding="utf-8-sig"
    )
